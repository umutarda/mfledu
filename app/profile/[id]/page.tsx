...
import { getProfileById, getProfileStats, getNotesByAuthor, getUser } from "@/lib/supabase/queries"
    ...
    useEffect(() => {
        async function loadProfile() {
            try {
                ...
                // If the user tries to view their own profile via ID, redirect them to the generic /profile page
                ...
                const [prof, st, notesRes] = await Promise.all([
                    ...
                    getProfileStats(id as string),
                    getNotesByAuthor(id as string),
                ])
        ...
        return (
            <div className="flex h-dvh overflow-hidden bg-background">
                ...
                <div className="flex flex-1 flex-col overflow-hidden">
                    ...
                    <main className="flex-1 flex flex-col items-center justify-center overflow-y-auto pb-20 lg:pb-0">
                        <h1 className="text-2xl font-bold mb-2">Kullanıcı Bulunamadı</h1>
                        <p className="text-muted-foreground mb-4">Aradığınız profil mevcut değil veya silinmiş olabilir.</p>
                        <Link href="/">
                            <Badge variant="outline" className="px-4 py-2 cursor-pointer hover:bg-muted">Ana Sayfaya Dön</Badge>
                        </Link>
                    </main>
                </div>
            </div>
                ...
                <main className="flex-1 overflow-y-auto pb-20 lg:pb-0">
                    <div className="mx-auto max-w-3xl px-4 py-6 lg:px-6">
                        ...
                        <h2 className="mb-3 text-base font-semibold text-foreground">
                            {profile?.username} kullanıcısının paylaştığı notlar ({userNotes.length})
                        </h2>
                        {userNotes.length > 0 ? (
